export function createPieChart(structure, div) {
    let pieArea = createPieArea(div)

    

    makeSinglePieChart(pieArea.summerContainer, {onPeak:66, offPeak: 33})
    makeSinglePieChart(pieArea.winterContainer, {onPeak:66, offPeak: 33})
}

function createPieArea(div) {
    let pieDiv = document.createElement('div')
    pieDiv.className = 'pie-container'
    div.appendChild(pieDiv)
    let header = document.createElement('h2')
    header.innerHTML = 'Percentage Breakdown'
