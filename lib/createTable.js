export function createTable(monthlyData) {
  ...
  export function renderTable(structure) {
    ...
    let tableHeaderString = `<thead><tr><td colspan="3">Bill Date Ranges</td><td colspan="2">Net Energy Use (KWH)</td></tr><tr><td>Start Date</td><td>End Date</td><td>Season</td><td>On Peak</td><td>Off Peak</td></tr></thead>`
    
    
    let bodyString = ``
    for(let monthly of structure.monthlies) {
      let rowString = `<tr><td>${monthly.month+1}/1/${monthly.year}</td><td>${monthly.month+1}/${monthly.endDate}/${monthly.year}</td><td>${monthly.season}</td><td>${monthly.onPeak}</td><td>${monthly.offPeak}</td></tr>`
